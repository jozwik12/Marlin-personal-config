...
#define CUSTOM_MACHINE_NAME "Józwikonator Mk7"
 ...
 */
  ...
  #define MPC_BLOCK_HEAT_CAPACITY { 11.07f }           // (J/K) Heat block heat capacities.
  #define MPC_SENSOR_RESPONSIVENESS { 0.1535f }         // (K/s per ∆K) Rate of change of sensor temperature from heat block.
  #define MPC_AMBIENT_XFER_COEFF { 0.0452f }           // (W/K) Heat transfer coefficients from heat block to room air with fan off.
  #if ENABLED(MPC_INCLUDE_FAN)
    #define MPC_AMBIENT_XFER_COEFF_FAN255 { 0.0608f }  // (W/K) Heat transfer coefficients from heat block to room air with fan on full.
  #endif
  ...
  #define MPC_TUNING_POS { X_CENTER, Y_CENTER, 0.40f } // (mm) M306 Autotuning position, ideally bed center at first layer height.
...
#define PIDTEMPBED
  ...
  #define DEFAULT_bedKp   87.15
  #define DEFAULT_bedKi   17.41
  #define DEFAULT_bedKd  290.78
...
#define Z_PROBE_SERVO_NR 0
  ...
  #define Z_SERVO_ANGLES { 55, 0 }      // Z Servo Deploy and Stow angles
 ...
 *     O-- FRONT --+
 */
#define NOZZLE_TO_PROBE_OFFSET { 26.2, -5.6, -1.40 }
...
#define PROBE_OFFSET_XMIN -40   // (mm)
#define PROBE_OFFSET_XMAX  40   // (mm)
#define PROBE_OFFSET_YMIN -40   // (mm)
#define PROBE_OFFSET_YMAX  40   // (mm)
...
//#define DEBUG_LEVELING_FEATURE
  ...
  //#define AVOID_OBSTACLES
    ...
    #define CLIP_W  30  // Bed clip width, should be padded a few mm over its physical size
    #define CLIP_H  11  // Bed clip height, should be padded a few mm over its physical size

    // Obstacle Rectangles defined as { X1, Y1, X2, Y2 }
    #define OBSTACLE1 { (X_BED_SIZE) / 2 - (CLIP_W) / 2, 0                              , (X_BED_SIZE) / 2 + (CLIP_W) / 2, CLIP_H }                      //bottom
    #define OBSTACLE2 { (X_BED_SIZE) / 2 - (CLIP_W) / 2, (Y_BED_SIZE) - (CLIP_H)        , (X_BED_SIZE) / 2 + (CLIP_W) / 2, Y_BED_SIZE }                  //top
    #define OBSTACLE3 { 0                              , (Y_BED_SIZE) / 2 - (CLIP_W) / 2, CLIP_H                         , (Y_BED_SIZE) / 2 + (CLIP_W) } //left
    #define OBSTACLE4 { (X_BED_SIZE) - (CLIP_H)        , (Y_BED_SIZE) / 2 - (CLIP_W) / 2, X_BED_SIZE                     , (Y_BED_SIZE) / 2 + (CLIP_W) } //right
...
// #define SKEW_CORRECTION
  ...
  #define XY_DIAG_BD 141.52
  #define XY_SIDE_AD 99.71
...
#define PREHEAT_4_LABEL       "Leveling"
#define PREHEAT_4_TEMP_HOTEND  130
...
#define NOZZLE_PARK_FEATURE
  ...
  #define NOZZLE_PARK_POINT { (X_MIN_POS + 1), (Y_MIN_POS + 1), 5 }
